...
public class TC4 extends BaseTest {
    ...
    public void vkTest() {
        ...
        String postId = WallWork.getWallPostId(secondUser, secondUserId, randomText);
        ...
        String commentId = WallWork.getWallCommentId(postId, secondUser);
        ...
        int liked = WallWork.getLikeStatus(secondUserId, secondUserId, postId, secondUser, Fields.POST);
        LOG.info("Checks if like status is 'LIKED'");
        Assert.assertEquals(liked, LikeStatus.LIKED.getValue(), String.format("User id=%s did not like item id=%s", secondUserId, postId));
        VkApiUtils.createWallPostDeleteRequest(secondUserId, postId, secondUser);
        LOG.info("Gets response from deleting wall post");
        Assert.assertFalse(myPage.getPost().isExists(secondUserId, postId), String.format("Post id=%s is not deleted", postId));
    }
}